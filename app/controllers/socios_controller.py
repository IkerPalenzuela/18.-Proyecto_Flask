...
def crear():
    ...
    if form.validate_on_submit():
        exito, mensaje = socio_service.crear_socio(
...
@role_required('admin')
def editar(id):
    socio = socio_service.obtener_por_id(id)
    ...
    if form.validate_on_submit():
        exito, mensaje = socio_service.editar_socio(
            id=id,
            nombre=form.nombre.data,
            email=form.email.data
        )
...
@role_required('admin')
def borrar(id):
    exito, mensaje = socio_service.borrar_socio(id)
    flash(mensaje, "success" if exito else "error")
    return redirect(url_for("socios.listar"))